...
Outputs are written under `proposals/openapi-publication/build/`, with one `openapi.json` and one `.well-known/x402` entry per service. The command is offline and does not publish files or make payments.
